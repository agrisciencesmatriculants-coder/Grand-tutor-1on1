...
/** The shared classroom stage: slides + George's live audio broadcast. */
export default function LiveStage() {
  ...
  useEffect(() => {
    ...
    client
      ...
      .maybeSingle()
      .then(({ data, error: err }) => {
        if (err) setError(`Could not read the stage: ${err.message}`);
        else if (data) {
  ...
  const setSlide = useCallback(
    async (file: File) => {
      ...
      const { error: updErr } = await supabase
        ...
        .eq('id', CLASSROOM_ID);
      if (updErr) setError(`Could not set the stage: ${updErr.message}`);
      setBusy(false);
    },
    [isTeacher],
  );
  ...
  const clearStage = async () => {
    ...
    const { error: updErr } = await supabase
      ...
      .eq('id', CLASSROOM_ID);
    if (updErr) setError(`Could not clear the stage: ${updErr.message}`);
  };
  ...
  const startLive = async () => {
    ...
    try {
      ...
      setError('Microphone access was denied — the stage mic stays off.');
    }
  };
  ...
  const stopLive = async () => {
    ...
    const { error: updErr } = await supabase
      ...
      .eq('id', CLASSROOM_ID);
    if (updErr) setError(`Could not sign off: ${updErr.message}`);
  };
  ...
  const runSessionSummary = useCallback(async () => {
    ...
    try {
      ...
      setEndPhase('done');
      setEndMessage('Summary sent to both inboxes');
    } catch (err) {
      setEndPhase('error');
  ...
  if (!supabase) {
    return (
      <div className="flex h-full items-center justify-center p-4">
        <p className="text-sm text-cream-dim">The stage lights come on once Supabase is configured.</p>
      </div>
    );
  }
    ...
    <div className="flex h-full min-h-0 flex-col" onPaste={onPasteZone}>
      ...
      <div className="flex flex-wrap items-center gap-2 px-1 pb-2">
        {liveOnAir && (
          <span
            className={cn(
              'font-label inline-flex items-center gap-1.5 rounded-full border border-crimson-light/70 bg-crimson/20 px-2.5 py-1 text-[10px] text-crimson-light',
              'animate-pulse',
            )}
          >
        ...
        {busy && <span className="font-label text-[10px] text-gold-light">Setting the stage…</span>}
        {endPhase === 'working' && (
          <span className="font-label inline-flex items-center gap-1.5 text-[10px] text-gold-light" role="status">
        ...
        {endPhase === 'done' && endMessage && (
          <span className="font-label text-[10px] text-gold-light" role="status">
            {endMessage}
          </span>
        )}
        {endPhase === 'error' && endMessage && (
          <span className="font-label text-[10px] normal-case text-crimson-light" role="alert">
            {endMessage}
          </span>
        )}
        ...
        {isTeacher && (
          <div className="ml-auto flex flex-wrap items-center gap-1.5">
            ...
            />
            <span
              className="font-label hidden rounded border border-dashed border-gold/30 px-2 py-1 text-[9px] text-cream-dim/70 sm:inline"
            ...
            <button
              ...
              className={cn(
                ...
                endPhase === 'confirm' &&
                  'border-crimson-light/70 bg-crimson/15 text-crimson-light hover:bg-crimson/25',
              )}
              title="Ask Agron to summarise this session and email it to George and Kelebogile"
            >
      ...
      {error && (
        <p className="mb-2 rounded-md border border-crimson/50 bg-crimson/10 px-2 py-1.5 text-xs text-crimson-light">
          {error}
        </p>
      )}

      {/* Stage area */}
      <div className="relative min-h-0 flex-1 overflow-hidden rounded-xl border border-gold/25 bg-stage-deep shadow-[inset_0_0_60px_rgba(0,0,0,0.55)]">
        <div className="spotlight" />
        {loading ? (
          <div className="flex h-full items-center justify-center">
            <p className="text-sm text-cream-dim">Dimming the house lights…</p>
          </div>
          ...
          <div className="relative z-10 flex h-full flex-col items-center justify-center gap-3 p-6 text-center">
            <img
              ...
              className="max-h-[55%] w-auto rounded-lg object-contain opacity-90"
            />
            <p className="font-display text-lg text-gold-light">
              {isTeacher
                ? 'Set the stage: upload or paste a slide'
                : 'The stage is dark — waiting for George'}
            </p>
          </div>
        )}
      </div>
        ...
        <>
          ...
          {needsTap && (
            <button
              type="button"
              onClick={tapToHear}
              className="mt-2 inline-flex items-center justify-center gap-2 rounded-lg border border-crimson-light/60 bg-crimson/20 px-3 py-1.5 text-xs text-crimson-light transition hover:bg-crimson/30"
            >
              <Radio className="h-3.5 w-3.5 animate-pulse" aria-hidden="true" />
              George is live — tap to hear
            </button>
          )}
        </>
      )}
