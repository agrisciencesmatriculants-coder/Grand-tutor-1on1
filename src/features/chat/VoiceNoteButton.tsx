...
/** Record/stop toggle button using MediaRecorder (audio/webm) with a pulsing terracotta state + timer. */
export default function VoiceNoteButton({ onRecorded, onError, disabled }: VoiceNoteButtonProps) {
  ...
  const start = async () => {
    ...
    try {
      ...
      releaseResources();
      setRecording(false);
      onError('Microphone access was denied. Voice notes cannot be recorded.');
    }
  };
  ...
  return (
    <div className="flex items-center gap-2">
      {recording && (
        <span className="font-label flex items-center gap-1.5 text-[10px] text-terra-deep">
          <span className="inline-block h-2 w-2 animate-ping rounded-full bg-terra" />
      ...
      <button
        ...
        className={cn(
          ...
          recording
            ? 'animate-pulse border-terra bg-terra/20 text-terra-deep shadow-[0_0_16px_rgba(180,102,74,0.4)]'
            : 'border-gold/40 bg-gold/10 text-gold-deep hover:bg-gold/20',
