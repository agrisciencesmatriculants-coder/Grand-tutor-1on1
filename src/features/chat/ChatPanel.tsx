...
function ThinkingBubble() {
  return (
    <div className="my-2 flex justify-center">
      <div className="rounded-xl border border-dashed border-gold/40 bg-stage-panel/60 px-4 py-2.5">
        <span className="font-label flex items-center gap-1 text-[10px] text-gold">
...
export default function ChatPanel() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (cancelled) return;
      if (err) {
        setError('Could not load the conversation. The curtain stuck — try refreshing.');
        setLoading(false);
        return;
      }
  ...
  const send = async () => {
    ...
    setSending(false);
    if (err) {
      setError('Message failed to send. Take it from the top.');
      return;
    }
    ...
    if (/@ai/i.test(text)) {
      ...
      thinkingTimeoutRef.current = window.setTimeout(() => {
        clearThinking();
        showAiError(`${AI_NAME} missed the cue — please try again.`);
      }, AI_TIMEOUT_MS);
      ...
      if (fnErr) {
        clearThinking();
        showAiError(`${AI_NAME} missed the cue — please try again in a moment.`);
      }
  ...
  const onFilePicked = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > MAX_FILE_SIZE) {
      setError('That prop is too heavy — files must be under 25MB.');
      return;
    }
    setUploading(true);
    setError(null);
    ...
    if (upErr) {
      setUploading(false);
      setError('Upload failed — the file never reached the stage.');
      return;
    }
    ...
    setUploading(false);
    if (insErr) {
      setError('File uploaded, but the message missed its entrance.');
      return;
    }
    if (data) appendIfNew(data as Message);
  };
  ...
  const onVoiceRecorded = async (blob: Blob) => {
    ...
    if (upErr) {
      setUploading(false);
      setError('Voice note failed to upload — the mic fumbled.');
      return;
    }
    const { data: urlData } = supabase.storage.from('files').getPublicUrl(path);
    const { data, error: insErr } = await supabase
    ...
    setUploading(false);
    if (insErr) {
      setError('Voice note uploaded, but the message missed its entrance.');
      return;
    }
    if (data) appendIfNew(data as Message);
  };
  ...
  if (!supabase) {
    return (
      <div className="card-playbill p-4 text-sm text-cream-dim">
        Chat is offstage until Supabase is configured.
      </div>
    );
  }
  if (!profile) {
    return (
      <div className="card-playbill p-4 text-sm text-cream-dim">
        Sign in to join the backstage chat.
      </div>
    );
  }
  ...
  return (
    <div className="card-playbill relative flex h-full min-h-[30rem] flex-col overflow-hidden">
      ...
      {/* Header */}
      <div className="relative z-10 flex items-center justify-between border-b border-gold/15 px-4 py-3">
        <div>
          <h2 className="font-display text-base font-bold text-gold-light">Backstage Chat</h2>
          <p className="font-label text-[9px] text-cream-dim/70">
            George · Kelebogile · {AI_NAME} — mention @ai for the tutor
          </p>
        </div>
        <span className="font-label hidden items-center gap-1 text-[9px] text-gold/60 sm:flex">
      ...
      <div className="relative z-10 flex-1 overflow-y-auto px-3 py-4 sm:px-4">
        {loading ? (
          <p className="font-display py-10 text-center text-sm text-cream-dim">
            Raising the curtain…
          </p>
        ) : messages.length === 0 ? (
          <div className="flex h-full min-h-[16rem] flex-col items-center justify-center gap-3 px-6 text-center">
            <img
              ...
              alt=""
              className="h-32 w-32 rounded-full border border-gold/20 object-cover opacity-80"
              onError={(e) => {
                (e.target as HTMLImageElement).style.display = 'none';
              }}
            />
            <p className="font-display flex items-center gap-2 text-lg text-gold-light">
              <MessagesSquare className="h-5 w-5 text-gold" aria-hidden="true" />
              The stage is set — say hello
            </p>
            <p className="max-w-xs text-xs text-cream-dim">
              George, Kelebogile and {AI_NAME} share this backstage channel. Break a leg.
            </p>
          </div>
        ) : (
              ...
              <Fragment key={message.id}>
                {showSeparator && (
                  <div className="my-3 flex items-center gap-3">
                    ...
                    <span className="font-label text-[9px] text-cream-dim/70">
        ...
        {aiError && (
          <p className="my-2 text-center font-label text-[10px] normal-case text-crimson-light">
            {aiError}
          </p>
        )}

        {typingName && (
          <p className="font-label mt-1 text-[10px] normal-case text-cream-dim/80">
      ...
      {/* Inline error */}
      {error && (
        <p className="relative z-10 border-t border-crimson/40 bg-crimson/10 px-4 py-1.5 text-center text-xs text-crimson-light">
          {error}
        </p>
      )}
      ...
      {/* Composer */}
      <form
        className="relative z-10 border-t border-gold/15 bg-stage-deep/60 px-3 py-2.5"
        onSubmit={(e) => {
          e.preventDefault();
          void send();
        }}
      >
        <div className="flex items-end gap-2">
          ...
          <button
            ...
            title="Attach a file"
            className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-gold/30 bg-gold/5 text-sm text-gold-light transition hover:bg-gold/15 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {uploading ? (
          ...
          <textarea
            ...
            onKeyDown={onKeyDown}
            placeholder="Take the stage… (Enter sends, Shift+Enter for a new line)"
