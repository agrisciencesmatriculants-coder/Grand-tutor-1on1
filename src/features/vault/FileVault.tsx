...
const FILTERS: Array<{ id: Filter; label: string }> = [
  ...
  { id: 'agron', label: 'Ready for Agron' },
];
...
export default function FileVault() {
  ...
  const fetchFiles = useCallback(async () => {
    ...
    if (err) {
      setError(`Could not load the files: ${err.message}`);
    } else {
  ...
  const uploaderName = (file: FileRow): string => {
    ...
    return profiles.get(file.uploader_id)?.full_name?.trim() || 'Member';
  };
  ...
  if (!supabase) {
    return (
      <div className="flex h-full items-center justify-center p-4">
        <p className="text-sm text-ink-soft">The file vault is available once Supabase is configured.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex items-center justify-between px-1 pb-2">
        <h2 className="font-label flex items-center gap-1.5 text-[11px] text-gold-deep">
          <FolderOpen className="h-3.5 w-3.5" aria-hidden="true" />
          File Vault
        </h2>
        {uploading && <span className="font-label text-[10px] text-gold-deep">Uploading…</span>}
      </div>
      ...
      <div
        ...
        className={cn(
          ...
          dragging
            ...
            : 'border-sand-deep bg-ivory-deep hover:border-gold/70 hover:bg-gold/10',
        )}
      >
        <p className="text-sm text-gold-deep">Drop files here, or click to browse</p>
      ...
      {error && (
        <p className="mt-2 rounded-md border border-terra/50 bg-terra-tint px-2 py-1.5 text-xs text-terra-deep">
          {error}
        </p>
      )}
        ...
        {FILTERS.map((f) => (
          <button
            ...
            className={cn(
              ...
              filter === f.id
                ? 'border-gold/70 bg-gold/15 text-gold-deep'
                : 'border-sand-deep text-ink-soft hover:border-gold/50 hover:text-ink',
            )}
          >
            {f.label}
          </button>
      ...
      <div className="mt-3 min-h-0 flex-1 space-y-1.5 overflow-y-auto pr-1">
        {loading ? (
          <p className="px-1 py-4 text-center text-xs text-ink-soft">Loading files…</p>
          ...
          visibleFiles.map((file) => (
            <div
              key={file.id}
              className="group flex items-center gap-2 rounded-lg border border-sand bg-paper px-2.5 py-2 transition hover:border-gold/50"
            >
              <button
                type="button"
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="flex items-center gap-1.5">
                    <span className="truncate text-sm text-ink">{file.file_name ?? 'Untitled'}</span>
                    {file.extracted_text && (
                      <span
                        className="h-1.5 w-1.5 shrink-0 rounded-full bg-sage shadow-[0_0_6px_rgba(125,140,92,0.8)]"
                        title="Ready for Agron"
                        aria-label="Ready for Agron"
                      />
                    )}
                  </span>
              ...
              {canDelete(file) && (
                <button
                  ...
                  className="shrink-0 rounded p-1 text-ink-soft/60 opacity-0 transition hover:bg-terra-tint hover:text-terra-deep group-hover:opacity-100"
      ...
      {preview && (
        ...
        >
          ...
          >
            ...
            <div className="min-h-0 flex-1 overflow-auto p-4">
              ...
              {previewKindOf(preview) === 'pdf' && previewUrl && (
                <iframe
                  ...
                  className="h-[65vh] w-full rounded-lg border border-sand bg-white"
                />
              )}
            ...
            <div className="flex items-center justify-between gap-3 border-t border-sand px-4 py-3">
